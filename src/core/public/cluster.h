...
public:
    ...
    FORCE_INLINE OMP::CriticalSection &getUsageGuard()
    { return usageGuard; }

    /// @brief Get distance from centroid
    FORCE_INLINE auto getDistance(const T &elem) const
    { return centroid.getDistance(elem); }

    /// @brief Add weight
    FORCE_INLINE void addWeight(const T &elem)
    {
        workingCentroid += elem;
        ++weight;
    }

    /// @brief Compare two clusters
    FORCE_INLINE bool operator==(const Cluster<T> &cluster)
    ...
    FORCE_INLINE Cluster &fuse(const Cluster<T> &cluster)
    { return operator+=(cluster); }

    FORCE_INLINE friend Cluster fuse(const Cluster<T> &a, const Cluster<T> &b)
    { return a.fuse(b); }
    /// @}
